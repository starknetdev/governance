...
VOTE_CERTIFICATE_CONTRACT_FILE = os.path.join("contracts", "VotingCertificate.cairo")
GOVERNOR_CONTRACT_FILE = os.path.join("contracts", "Governor.cairo")
ZODIAC_RELAYER_CONTRACT_FILE = os.path.join(
    "contracts/execution", "zodiac_relayer.cairo"
)
TARGET_CONTRACT_FILE = os.path.join("contracts/execution", "Target.cairo")
BASIC_VOTING_STRATEGY_CONTRACT_FILE = os.path.join(
    "contracts/strategies", "vanilla.cairo"
)
SHARE_VOTING_STRATEGY_CONTRACT_FILE = os.path.join(
    "contracts/strategies", "ERC721_voting.cairo"
)
...
async def contract_factory():
    ...
    await signer1.send_transaction(
        ...
        selector_name="mint",
        calldata=[account1.contract_address, *TEST_SHARE, TEST_FUND],
    )

    zodiac_relayer = await starknet.deploy(
        source=ZODIAC_RELAYER_CONTRACT_FILE, constructor_calldata=[]
    )

    target = await starknet.deploy(source=TARGET_CONTRACT_FILE, constructor_calldata=[])

    basic_voting_strategy = await starknet.deploy(
        source=BASIC_VOTING_STRATEGY_CONTRACT_FILE, constructor_calldata=[]
    )
